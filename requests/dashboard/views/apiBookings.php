...
if( isset($_GET["action"]) && !empty($_GET["action"]) ){
    ...
    if( $action == "list" ){
        if( $Bookings = selectDB2New("`vendorId`, `branchId`, `serviceId`, `extras`, `extraInfo`, `bookedTime`, `bookedDate`, JSON_UNQUOTE(JSON_EXTRACT(customerDetails, '$.name')) AS customer_name, JSON_UNQUOTE(JSON_EXTRACT(customerDetails, '$.mobile')) AS customer_mobile, JSON_UNQUOTE(JSON_EXTRACT(customerDetails, '$.email')) AS customer_email, `chargeType`,FORMAT(totalPrice, 3) AS `totalPrice`, `status`","bookings",[$data["vendorId"]],"`vendorId` = ?","") ){
