...
if( isset($_GET["action"]) && !empty($_GET["action"]) ){
    ...
    if( $action == "list" ){
        if( $Bookings = selectDB2New("`vendorId`, `branchId`, `serviceId`, `extras`, `extraInfo`, `bookedTime`, `bookedDate`, JSON_OBJECT( 'name', JSON_UNQUOTE(JSON_EXTRACT(customerDetails, '$.name')), 'mobile', JSON_UNQUOTE(JSON_EXTRACT(customerDetails, '$.mobile')), 'email', JSON_UNQUOTE(JSON_EXTRACT(customerDetails, '$.email')) ) AS customerDetails, `chargeType`,FORMAT(totalPrice, 3) AS `totalPrice`, `status`","bookings",[$data["vendorId"]],"`vendorId` = ?","") ){
