...
if( isset($_GET["action"]) && !empty($_GET["action"]) ){
    ...
    if( $action == "list" ){
        if( $Bookings = selectDB2New("`vendorId`, `branchId`, `serviceId`, `extras`, `extraInfo`, `bookedTime`, `bookedDate`, JSON_PARSE(JSON_UNQUOTE(JSON_VALUE(data, '$.customerDetails'))), `chargeType`,FORMAT(totalPrice, 3) AS `totalPrice`, `status`","bookings",[$data["vendorId"]],"`vendorId` = ?","") ){
